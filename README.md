...
At first, I had thought of making the level so that it was all horizontal and that the player would have had to run back to the beginning after getting all the keys but that would have been a boring experience since they would just go through everything that they had already passed. ![First Map Layout](DocImages/FirstMapLayout.jpg) I thought about moving the end dorr to the end or the level but that felt like it limited my options a little since a fully horizontal level could use more vertical sections for things like secret rooms or areas to challenge the player further. Although there are some parts in the level that may feel a little boring since sometimes there is nothing happening at that point so maybe I would need to add a few more enemies or parkour to those sections to liven things up a little bit more. Some changes with the level and level map are the gun, key and health pickup in section 1 since I thought it would be a little more interesting if I added moving platforms to upper areas where the player can discover that they might need to go to these areas if they want to progress the level. For section 2 and 3, I decided to change some things up and add or move around the health pickups so that they were in a sort of challenge room or reward room for the player to find health or to kill enemies which are usually the underground areas where the players has to pass by a passable platform so it makes a sort of pattern to do check those underground platforms. I think if I had iterated a bit more, I could have made more of the level vertically so that would be a different kind of experience than most of my current design but I did figure out a way to make the end door stay at the end since all I had to do was make the level loop around but not giving the player access to section 3 at the start. ![Final Map Layout](DocImages/FinalMapLayout.jpg)I think section 3 needs a bit more iterating to see if the section is too hard or easy since it feels a bit lacking and it might be because of the length of section 3 being quite long and I did not fit in enough enemies or acid.
